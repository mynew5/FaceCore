...
enum eGurgthock
{
    ...
    SAY_QUEST_ACCEPT_VLADOF                       = -1571048,
                ...
                if (uiTimer <= uiDiff)
                {
                    switch (uiPhase)
                    {
                        ...
                        case 14:
                            ...
                            break;
                        case 16:
                            if (Creature* summon = me->SummonCreature(NPC_VLADOF, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, 1000))
                                SummonGUID = summon->GetGUID();
                            DoScriptText(SAY_QUEST_ACCEPT_VLADOF, me);
                            uiPhase = 0;
                            break;
                    }
                }else uiTimer -= uiDiff;
